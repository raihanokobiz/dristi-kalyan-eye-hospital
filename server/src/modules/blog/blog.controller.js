...
class BlogController {
  ...
  getBlogBySlug = catchError(async (req, res, next) => {
    const slug = req.params.slug;
    const blogResult = await BlogService.getBlogBySlug(slug);
    const resDoc = responseHandler(200, "Blog fetched successfully", blogResult);
    res.status(resDoc.statusCode).json(resDoc);
  });
