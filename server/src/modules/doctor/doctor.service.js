...
class DoctorService {
    async createDoctor(payloadFiles, payload, session) {
        ...
        if (!name || !degree || !visitingTime || !phoneNumber || !availableDays || !consultationFee) {
    ...
    async updateDoctor(id, payloadFiles, payload) {
        const { files } = payloadFiles;
        ...
        // Remove old profile picture if it's being replaced
        if (files?.length && oldDoctorData?.profilePicture) {
            await removeUploadFile(oldDoctorData.profilePicture);
        }
