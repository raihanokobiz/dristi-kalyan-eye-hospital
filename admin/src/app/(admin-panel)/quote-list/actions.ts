...
import { deleteQuote, updateQuoteStatus } from "./quote";
import { SteadfastOrderPayload } from "@/types/shared";
import { revalidatePath } from "next/cache";

export async function UpdateOrderStatus(quoteId: string, status: string) {
  try {
    const statusEntry = { status: status };
    const res = await updateQuoteStatus(String(quoteId), statusEntry);
